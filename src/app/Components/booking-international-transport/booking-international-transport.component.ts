...
export class BookingInternationalTransportComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private bookingService: BookingInternationalTransportService) { }
  ...
  ngOnInit(): void {
    ...
    const departureDateObj = new Date(this.selectedDepartureFlight.departureDate);
    const arrivalDateObj = new Date(this.selectedDepartureFlight.arrivalDate);
    const returnDateObj = this.selectedDepartureFlight.returnDate ? new Date(this.selectedDepartureFlight.returnDate) : undefined;
    ...
    this.bookingForm = this.fb.group({
      ...
      departureBooking: this.fb.group({
        ...
        numberOfSeats: [1, [Validators.required, Validators.min(1), Validators.max(this.selectedDepartureFlight.availableSeats)]]
      })
  ...
  submitBooking() {
   ...
   console.log('Return Booking DTO:', retBooking);
    ...
    this.bookingService.bookTransport(depBooking).subscribe({
      ...
      error: (err) =>
      {
        console.log('Error booking departure flight:', err);
